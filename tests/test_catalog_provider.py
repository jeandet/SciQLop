...
def test_event_range_change_preserves_sort_order(qtbot, qapp):
    """Reproducer: after changing an event's start time (drag-edit), the internal
    list must stay sorted so that bisect range queries return correct results."""
    from SciQLop.components.catalogs.backend.provider import CatalogEvent
    provider = _make_dummy_provider(qapp)
    cat = provider.catalogs()[0]

    # Events are day 0..99. Move event at day 50 to day 5.5 (between day 5 and 6).
    events_before = provider.events(cat)
    evt_50 = events_before[50]
    new_start = datetime(2020, 1, 6, 12, tzinfo=timezone.utc)
    evt_50.start = new_start

    # Query a range that should include the moved event
    query_start = datetime(2020, 1, 6, tzinfo=timezone.utc)
    query_stop = datetime(2020, 1, 7, tzinfo=timezone.utc)
    results = provider.events(cat, start=query_start, stop=query_stop)
    result_uuids = {e.uuid for e in results}

    assert evt_50.uuid in result_uuids, (
        f"Moved event {evt_50.uuid} (start={evt_50.start}) not found in "
        f"range query [{query_start}, {query_stop}]. "
        f"Got {len(results)} results: {[e.uuid for e in results]}"
    )

    # Also verify the full list is sorted
    all_events = provider.events(cat)
    starts = [e.start for e in all_events]
    assert starts == sorted(starts), "Event list is not sorted after range change"
