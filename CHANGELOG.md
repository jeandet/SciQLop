...
### User API hardening (fuzzing campaign 2026-06-12)

- Fixed all graphic primitives (`Text`, `Ellipse`, `CurvedLine`, `Pixmap`, `HorizontalLine`, `add_hline`) failing with a cryptic `NameError` on plots obtained through `panel.plots` enumeration: those wrappers held a `SciQLopPlotInterfacePtr` smart pointer that the C++ item constructors reject. Plot wrappers now dereference the pointer to the concrete plot at construction, so every public path yields identical objects.
- `plot_product` with an unknown or invalid product now raises a clear `ValueError` (including the `//`-joined path and a hint that paths use Products-panel display names) instead of an internal `TypeError: cannot unpack non-iterable NoneType`.
- `TimeSeriesPlot`/`XYPlot`/`ProjectionPlot` gained a real `plot_type` property (the protocol stub silently returned `None`); on destroyed plots it raises the same "does not exist anymore" `ValueError` as other plot methods.
- A `plot_function` callback that raises now shows the error on the plot's in-canvas overlay (Error level, cleared on the next successful fetch) instead of only printing a console traceback while the plot stayed empty forever.
- `panel.save(path)` now raises `OSError` when the underlying exporter fails (missing directory, no write permission) instead of silently writing nothing.
- `panel.time_range` rejects non-finite bounds (NaN corrupted the panel to 1970) and zero-width ranges (what `TimeRange('garbage', …)` silently parses to).
...
Round 2 (`docs/api-fuzzing-report-round2-2026-06-12.md`):

- `catalogs.save`/`create`/`add_events` now reject events whose start is after their stop (mirroring tscat's rule; `start == stop` stays allowed). Such events used to be accepted into the user-api cache while the tscat store rejected them asynchronously on its driver thread — the two stores silently disagreed about catalog contents afterwards.
- `plot.remove_graph(graph)` now refuses to remove a graph that belongs to another plot (`ValueError`) — it used to destroy the other plot's graph — and raises `TypeError` on non-graph arguments instead of a raw `AttributeError`.
